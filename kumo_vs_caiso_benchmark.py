...
- 📊 Proper baseline: 354.1 MW MAE target (from independent validation)
...
class GeographicallyAlignedDataLoader:
    ...
    - Target: Beat 354.1 MW MAE (from independent holdout validation)
...
class TemporalKumoTrainer:
    ...
    def compare_temporal_forecasts_with_caiso(self, caiso_data: pd.DataFrame, temporal_results: Dict[str, Any]):
        """Compare temporal Kumo forecasts with corresponding CAISO forecast horizons using config."""
        print("\n  ⚔️ TEMPORAL FORECAST COMPARISON: Kumo vs CAISO")
        print("  " + "="*60)
        
        # Get CAISO benchmark metrics from config
        caiso_benchmarks = self.config.get_caiso_benchmarks()
        
        # CRITICAL UPDATE: Use our geographically aligned baselines from independent validation
        # These are the REAL targets to beat (not the original misaligned benchmarks)
        sf_bay_area_targets = {
            '7DA': {'mae_mw': 357.9, 'rmse_mw': 433.5, 'mape_percent': 12.9},
            '2DA': {'mae_mw': 354.1, 'rmse_mw': 427.9, 'mape_percent': 12.9},  # PRIMARY TARGET
            'DAM': {'mae_mw': 356.2, 'rmse_mw': 427.7, 'mape_percent': 13.0}
        }
        
        print("  🌉 Using SF Bay Area (POS 3.8) aligned baselines:")
        for horizon, metrics in sf_bay_area_targets.items():
            print(f"     {horizon}: {metrics['mae_mw']:.1f} MW MAE target")
        print("  🎯 PRIMARY TARGET: Beat 354.1 MW MAE (2DA forecast)")
        print()
        ...
        for horizon in ['7DA', '2DA', 'DAM']:
            ...
            print(f"      SF Bay Area {horizon} TARGET: MAE={sf_bay_area_targets[horizon]['mae_mw']:.1f}MW, MAPE={sf_bay_area_targets[horizon]['mape_percent']:.1f}%")
            
            horizon_results = temporal_results[horizon]
            best_mae = float('inf')
            best_result = None
            
            # Find best performing sample size for this horizon
            for result_key, result_data in horizon_results.items():
                if 'mae' in result_data and result_data['mae'] is not None:
                    mae = result_data['mae']
                    ...
                    # Calculate metrics
                    if 'actual_values' in result_data and 'predicted_values' in result_data:
                        actual_vals = result_data['actual_values']
                        pred_vals = result_data['predicted_values']
                        
                        if len(actual_vals) == len(pred_vals) and len(actual_vals) > 0:
                            ...
                            print(f"      Kumo {horizon} (n={sample_size}): MAE={mae:.1f}MW, RMSE={rmse:.1f}MW, MAPE={mape:.1f}%")
            
            # Compare best Kumo result with SF Bay Area target for this horizon
            if best_result:
                best_kumo_results[horizon] = best_result
                target_mae = sf_bay_area_targets[horizon]['mae_mw']
                
                improvement = ((target_mae - best_mae) / target_mae) * 100
                status = "🏆 BEATS TARGET" if improvement > 0 else "❌ NEEDS WORK"
                
                print(f"      📈 BEST Kumo {horizon}: {status} ({improvement:+.1f}% vs SF Bay Area target)")
                
                if improvement > 0:
                    print(f"      🎉 Kumo {horizon} BEATS SF BAY AREA TARGET by {improvement:.1f}%!")
                else:
                    gap = abs(improvement)
                    print(f"      🎯 Kumo {horizon} needs {gap:.1f}% improvement to beat SF Bay Area target")
        
        # Overall summary
        print(f"\n  🏆 OVERALL TEMPORAL FORECAST PERFORMANCE:")
        
        horizons_beaten = []
        total_improvement = 0
        
        for horizon, kumo_result in best_kumo_results.items():
            if kumo_result and 'mae' in kumo_result:
                kumo_mae = kumo_result['mae']
                target_mae = sf_bay_area_targets[horizon]['mae_mw']
                improvement = ((target_mae - kumo_mae) / target_mae) * 100
                
                if improvement > 0:
                    horizons_beaten.append(horizon)
                
                total_improvement += improvement
                print(f"     {horizon}: Kumo MAE={kumo_mae:.1f}MW vs SF Bay Area TARGET={target_mae:.1f}MW ({improvement:+.1f}%)")
        
        if horizons_beaten:
            print(f"\n  🎊 KUMO BEATS SF BAY AREA TARGETS IN {len(horizons_beaten)}/3 FORECAST HORIZONS: {', '.join(horizons_beaten)}")
            print(f"  🌉 These are REAL wins - geographically aligned comparison!")
        else:
            print(f"\n  🎯 KUMO NEEDS IMPROVEMENT IN ALL SF BAY AREA FORECAST HORIZONS")
            avg_gap = -total_improvement / len(best_kumo_results) if best_kumo_results else 0
            print(f"     Average improvement needed: {avg_gap:.1f}%")
            print(f"  🎯 PRIMARY GOAL: Beat 354.1 MW MAE for 2DA forecasting")
        
        print(f"\n  💡 KEY INSIGHT: This is now a FAIR comparison!")
        print(f"     ✅ Kumo 7DA predicts Day X+7 using Day X features (same as CAISO 7DA)")
        print(f"     ✅ Kumo 2DA predicts Day X+2 using Day X features (same as CAISO 2DA)")  
        print(f"     ✅ Kumo DAM predicts Day X+1 using Day X features (same as CAISO DAM)")
        print(f"     🌉 Geographic alignment: SF Bay Area features → SF Bay Area load (POS 3.8)")
        print(f"     🎯 Real targets from independent holdout validation (not circular benchmarks)")
...
def main(config_path: str = "config.yaml"):
    ...
    print("=" * 70)
    print("🎯 Target: Beat 354.1 MW MAE (from independent validation)")
    ...
    try:
        ...
        print(f"  🎯 Target to beat: 354.1 MW MAE (from independent holdout validation)")
